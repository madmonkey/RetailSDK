        ...
        public class CartWebOperationsHandler
        {
            ...
            public async Task<Cart> UpdateItems(bool isCheckoutSession, IEnumerable<CartLine> cartLines)
            {
                ...
                if (sessionType == SessionType.AnonymousCheckout || sessionType == SessionType.SignedInCheckout)
                {
                    ...
                    // Be sure to pass the latest CartLines from the previous update--
                    // the CartLines passed by the caller may be outdated, since we already ran UpdateItems once
                    await cartOperationsHandler.UpdateItems(cartId, cart.CartLines);
                }
            ...
            /// </summary>
            /// <param name="isRedirect"> Boolean value to mentioned is this checkout is after a redirect.</param>
            /// <returns>Checkout cart.</returns>
            public async Task<Cart> CommenceCheckout(bool isRedirect)
            {
                EcommerceContext ecommerceContext = ServiceUtilities.GetEcommerceContext(this.HttpContextBase);
                CartOperationsHandler cartOperationsHandler = new CartOperationsHandler(ecommerceContext);
                ...
                if (isRedirect)
                {
                    checkoutCart = await cartOperationsHandler.GetCart(previousCheckoutCartId);
                }
                else
                {
                    checkoutCart = await cartOperationsHandler.CommenceCheckout(shoppingCartId, previousCheckoutCartId);
                }
            ...
            public async Task<Cart> GetCart(bool isCheckoutSession)
            {
                ...
                else
                {
                    ...
                    else if (sessionType == SessionType.SignedInShopping)
                    {
                        ...
                        Cart activeAuthenticatedShoppingCart = await customerOperationsHandler.GetActiveShoppingCart();
