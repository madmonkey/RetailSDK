        ...
        public class ProductDetailsController : ActionControllerBase
        {
            ...
            public async Task<ActionResult> Index(string productId = "")
            {
                ...
                else
                {
                    ...
                    IEnumerable<long> productCatalogs = await productOperationsHandler.GetProductCatalogs(Utilities.DefaultQuerySettings);

                    PagedResult<Product> products = await productOperationsHandler.SearchProducts(searchCriteria, productCatalogs, Utilities.DefaultQuerySettings);
