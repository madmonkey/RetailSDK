...
namespace Contoso
{
    namespace Retail.Ecommerce.Web.Storefront.Controllers
    {
        ...
        public class CustomerController : WebApiControllerBase
        {
            ...
            /// </summary>
            /// <param name="queryResultSettings">The query result settings.</param>
            /// <returns>An order shipments response.</returns>
            public async Task<ActionResult> GetOrderShipmentsHistory(QueryResultSettings queryResultSettings)
            {
                EcommerceContext ecommerceContext = ServiceUtilities.GetEcommerceContext(this.HttpContext);
                CustomerOperationsHandler customerOperationsHandler = new CustomerOperationsHandler(ecommerceContext);
                PagedResult<OrderShipments> orderShipments = await customerOperationsHandler.GetOrderShipmentsHistory(queryResultSettings);

                return this.Json(orderShipments);
            }
            ...
            /// <summary>
            /// Gets the wish lists for a given customer.
            /// </summary>
            /// <param name="queryResultSettings">The query result settings.</param>
            /// <returns>A response containing the customer's wish lists.</returns>
            public async Task<ActionResult> GetWishLists(QueryResultSettings queryResultSettings)
            {
                EcommerceContext ecommerceContext = ServiceUtilities.GetEcommerceContext(this.HttpContext);
                CustomerOperationsHandler customerOperationHandler = new CustomerOperationsHandler(ecommerceContext);
                PagedResult<CommerceList> wishLists = await customerOperationHandler.GetWishLists(queryResultSettings);

                return this.Json(wishLists.Results);
            }
        }
    }
}