...
namespace Contoso.Commerce.Client.Data.Services.RetailServer
{
    ...
    internal static class IEnumerableExtensions
    {
        ...
        public static PagedResult<T> ToCRTPagedResult<T>(this ReadOnlyCollection<T> source) where T : class
        {
            return new PagedResult<T>(source);
        }
        ...
        /// <param name="source">The source.</param>
        /// <returns>The PagedResult object.</returns>
        public static PagedResult<T> ToCRTPagedResult<T>(this IEnumerable<T> source) where T : class
        {
            var collection = new ReadOnlyCollection<T>(new List<T>(source));
            return new PagedResult<T>(collection);
        }
    }
}
