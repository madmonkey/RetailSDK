...
namespace Contoso
{
    namespace RetailServer.TestClient
    {
        ...
        internal partial class MainForm : Form
        {
            ...
            private static Guid userSessionID;
            ...
            [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance", "CA1823:AvoidUnusedPrivateFields", Justification = "Suppressed since its a known issue.  Bug 3907331 has been logged for this issue.")]
            ...
            private void RefreshActivationStorageInformation()
            {
                ...
                else
                {
                    ...
                    ICommerceRuntimeExtensionsSection extensionsSection = (ICommerceRuntimeExtensionsSection)ConfigurationManager.GetSection("commerceRuntimeExtensions");
                    ...
                    IRetailProxyExtensionsSection retailProxyExtensionsSection = (IRetailProxyExtensionsSection)ConfigurationManager.GetSection("retailProxyExtensions");
                    RetailProxyConfiguration retailProxyConfiguration = new RetailProxyConfiguration(retailProxyExtensionsSection);

                    CommerceAuthenticationRuntimeProvider authenticationProvider = new CommerceAuthenticationRuntimeProvider();

                    context = new CommerceRuntimeContext(_ => commerceRuntimeConfiguration, () => retailProxyConfiguration, null, authenticationProvider);
                    this.currentManagerFactory = ManagerFactory.Create(context);
                    context.SetDeviceToken(deviceToken);
                }
                
                // Set the AppSessionId for the Retail Server Context.
                this.currentManagerFactory.Context.SetAppSessionId(MainForm.AppSessionID);
            }
            ...
            private void RefreshLogOnInformation()
            {
                if (this.IsActivated && this.currentManagerFactory.Context.GetUserToken() != this.currentUserToken)
                {
                    ...
                    // Set the the UserSessionId.
                    if (this.currentManagerFactory.Context.GetUserToken() != null)
                    {
                        MainForm.userSessionID = Guid.NewGuid();
                        this.currentManagerFactory.Context.SetUserSessionId(MainForm.userSessionID);
                    }
                }
            }
            ...
            private async void RefreshUI()
            {
                ...
                if (this.IsLoggedOn)
                {
                    ...
                    this.listViewLogonDetails.Items.Add(string.Format("AppSessionID: {0}, UserSessionID: {1}", MainForm.AppSessionID, MainForm.userSessionID));
                }
                else
                {
            ...
            private /* async */ void RunSdkSampleTests()
            {
                try
                {
                    ...
                    CustomerSearchCriteria sc = new CustomerSearchCriteria();
                    sc.Keyword = "Contoso";
                    sc.SearchLocationValue = (int)SearchLocation.Local;

                    // query for customers 
                    ICustomerManager microsoftCustomerManager = this.currentManagerFactory.GetManager<ICustomerManager>();
                    var globalCustomers = await microsoftCustomerManager.Search(sc, queryResultSettings);
                    this.Log("Found {0} customers.", globalCustomers.Count());

                    foreach (var globalCustomer in globalCustomers.Results)
                    {
                        var customer = await microsoftCustomerManager.Read(globalCustomer.AccountNumber);
                        ...
                        customer.ExtensionProperties.Add(emailOptInProperty);
                        ...
                        var updatedCustomer = await microsoftCustomerManager.Update(customer);
                        this.Log("Customer '{0}' updated with ExtensionProperty EMAILOPTIN={1}.", customer.AccountNumber, emailOptInPropertyValue);
                    }
                    ...
                    Contoso.Commerce.RetailProxy.CrossLoyaltySample.ICustomerManager contosoCustomerManager
                        = this.currentManagerFactory.GetManager<Contoso.Commerce.RetailProxy.CrossLoyaltySample.ICustomerManager>();
                    decimal discount = await contosoCustomerManager.GetCrossLoyaltyCardDiscountAction("425-999-4444");
                    ...
                    Contoso.Commerce.RetailProxy.StoreHoursSample.IStoreDayHoursManager storeDayHoursManager
                        = this.currentManagerFactory.GetManager<Contoso.Commerce.RetailProxy.StoreHoursSample.IStoreDayHoursManager>();
                    var result = await storeDayHoursManager.GetStoreDaysByStore("HOUSTON", queryResultSettings);
                    IEnumerable<Contoso.Commerce.RetailProxy.StoreHoursSample.StoreDayHours> storeDayHours = result.Results;
