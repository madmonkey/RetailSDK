...
namespace Contoso
{
    namespace Commerce.Runtime.SequentialSignatureRegister.Configuration
    {
        ...
        /// </summary>
        public class SequentialSignatureRegisterConfiguration
        {
            /// <summary>
            /// Represents the sales transaction signature section name in the configuration file.
            /// </summary>
            private const string SequentialSignatureRegisterConfigSectionName = "SequentialSignatureRegister";
            ...
            private static readonly Lazy<SequentialSignatureRegisterConfiguration> ConfigInstance = new Lazy<SequentialSignatureRegisterConfiguration>(() => new SequentialSignatureRegisterConfiguration());
            ...
            private SequentialSignatureRegisterConfiguration()
            {
                try
                {
                ...
                {
                    throw new SequentialSignatureRegisterConfigurationException(ex.Message, ex);
                }
            }
            ...
            /// </value>
            public SequentialSignatureRegisterConfigSection SalesTransactionSignatureConfigSection { get; private set; }

            /// <summary>
            /// Gets the single instance of the <see cref="SequentialSignatureRegisterConfiguration"/> class.
            /// </summary>
            ...
            public static SequentialSignatureRegisterConfiguration GetInstance(string cultureName)
            {
            ...
            private SequentialSignatureRegisterConfigSection GetSalesTransactionSignatureConfigSectionFromAssemblyConfiguration()
            {
                SequentialSignatureRegisterConfigSection salesTransactionSignatureConfigSection = null;

                var executingAssemblyCodeBase = typeof(SequentialSignatureRegisterConfigSection).GetTypeInfo().Assembly.CodeBase;
                ...
                {
                    var exceptionMessage = SequentialSignatureRegisterLocalizer.Instance.Translate(currentCultureName, FileNotFoundExceptionMessageResourceId);
                ...
                salesTransactionSignatureConfigSection = configuration.GetSection(SequentialSignatureRegisterConfigSectionName) as SequentialSignatureRegisterConfigSection;

                if (salesTransactionSignatureConfigSection == null)
                {
                    var exceptionMessage = SequentialSignatureRegisterLocalizer.Instance.Translate(currentCultureName, SectionNotFoundExceptionMessageResourceId);
                    throw new ConfigurationErrorsException(string.Format(exceptionMessage, SequentialSignatureRegisterConfigSectionName, ConfigSectionsName));
                }

                if (string.IsNullOrEmpty(salesTransactionSignatureConfigSection.CertificateThumbprint))
                {
                    var exceptionMessage = SequentialSignatureRegisterLocalizer.Instance.Translate(currentCultureName, SectionShouldContainsThumbprintExceptionMessageResourceId);
                ...
                {
                    var exceptionMessage = SequentialSignatureRegisterLocalizer.Instance.Translate(currentCultureName, SectionShouldContainsStoreNameAndLocationExceptionMessageResourceId);
                    throw new ConfigurationErrorsException(exceptionMessage);
                }
                ...
                {
                    var exceptionMessage = string.Format(SequentialSignatureRegisterLocalizer.Instance.Translate(currentCultureName, SectionShouldContainsValidAttributeValueExceptionMessageResourceId), CertificateStoreLocationAttributeName);
                    throw new ConfigurationErrorsException(exceptionMessage);
                }
                ...
                {
                    var exceptionMessage = string.Format(SequentialSignatureRegisterLocalizer.Instance.Translate(currentCultureName, SectionShouldContainsValidAttributeValueExceptionMessageResourceId), CertificateStoreNameAttributeName);
                    throw new ConfigurationErrorsException(exceptionMessage);
                }
