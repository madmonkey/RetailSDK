...
namespace Contoso
{
    namespace Commerce.Runtime.SalesTransactionSignatureNorway
    {
        ...
        /// </summary>
        public static class SalesTransactionSigningStrategy
        {
            /// <summary>
            ...
            public static IEnumerable<SalesLine> GetSaleLinesForSigning(SalesTransaction salesTransaction)
            {
                ...
                return salesTransaction.ActiveSalesLines
                    .Where(line => !line.IsGiftCardLine && !SalesTransactionSigningStrategy.IsSkipReportsLine(line))
                    .Where(line => SalesTransactionSigningStrategy.IsNormalLineValid(salesTransaction, line) || SalesTransactionSigningStrategy.IsPostedLineValid(salesTransaction, line));
            }
            ...
            public static bool IsSigningRequired(SalesTransaction salesTransaction)
            {
                ThrowIf.Null(salesTransaction, "salesTransaction");

                // perform signing only if there are lines to sign 
                bool hasSaleLinesForSigning = SalesTransactionSigningStrategy.GetSaleLinesForSigning(salesTransaction).Any();
            ...
            private static bool IsSkipReportsLine(SalesLine line)
            {
