...
namespace Contoso
{
    namespace Retail.Ecommerce.Sdk.Core.OperationsHandlers
    {
        ...
        public class RetailOperationsHandler : OperationsHandlerBase
        {
            ...
            /// <param name="giftCardExpirationYear">The gift card expiration.</param>
            /// <param name="giftCardExpirationMonth">The gift card expiration.</param>
            /// <returns>A response containing gift card balance.</returns>
            public virtual async Task<GiftCard> GetGiftCardInformation(string tenderTypeId,
                ...
                int giftCardExpirationMonth)
            {
                ManagerFactory managerFactory = Utilities.GetManagerFactory(this.EcommerceContext);
                IStoreOperationsManager storeOperationsManager = managerFactory.GetManager<IStoreOperationsManager>();
                GiftCard giftCard = await storeOperationsManager.GetGiftCard(giftCardNumber, tenderTypeId: tenderTypeId, pin: giftCardPin, expirationMonth: giftCardExpirationMonth, expirationYear: giftCardExpirationYear);
            ...
            public virtual async Task<CardPaymentAcceptResult> RetrieveCardPaymentAcceptResult(string cardPaymentResultAccessCode, string returnUrl, string cartId)
            {
                ManagerFactory managerFactory = Utilities.GetManagerFactory(this.EcommerceContext);
                ICartManager cartManager = managerFactory.GetManager<ICartManager>();
                RetrievePaymentResultSettings resultSettings = new RetrievePaymentResultSettings
                {
                    ReturnUrl = returnUrl
                };

                CardPaymentAcceptResult cardPaymentAcceptResult = await cartManager.RetrieveCardPaymentAcceptResult(cardPaymentResultAccessCode, null, cartId, resultSettings);
