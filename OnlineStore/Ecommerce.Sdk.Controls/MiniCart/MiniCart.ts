...
module Contoso.Retail.Ecommerce.Sdk.Controls {
    ...
    export class MiniCart {
        ...
        private getCartProductsCount(cartLines: CommerceProxy.Entities.CartLine[]): number {
            var productCount: number = 0;

            if (Utils.hasElements(cartLines)) {
                for (var i: number = 0; i < cartLines.length; i++) {
                    productCount += cartLines[i].Quantity;
                }
            }
