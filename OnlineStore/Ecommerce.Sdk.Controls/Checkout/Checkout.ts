...
module Contoso.Retail.Ecommerce.Sdk.Controls {
    ...
    export class Checkout {
        ...
        private countriesRegions: ObservableArray<{ CountryCode: string; CountryName: string }>;
        ...
        private giftCard: ObservableArray<{
            giftCardAmount: Observable<number>,
            giftCardNumber: Observable<string>,
            giftCardPin: Observable<string>,
            giftCardExpirationYear: Observable<number>,
            giftCardExpirationMonth: Observable<number>,
            giftCardBalance: Observable<string>,
            isGiftCardInfoAvailable: Observable<boolean>,
            formattedGiftCardAmount: Observable<string>,
            showBalance: Observable<boolean>,
            showPin: Observable<boolean>,
            showExpiration: Observable<boolean>,
            tenderTypeId: Observable<string>,
            cardTypeId: Observable<string>
        }>;
        ...
        // Indicates that a call to retrieveCardPaymentAcceptResult revealed an additional action is needed on the connector side, it might be extra redirect or a challenge presented to a user.
        private static CARDPAYMENTACCEPTADDITIONALCONTEXT: string = "msax-cc-extracontext";

        // Used by the payment connector to redirect the customer to an external page to continue required interaction.
        private static CARDPAYMENTACCEPTREDIRECT: string = "msax-cc-redirect";
        ...
        private sessionStorageKeys = {
            PaymentCardAdddress: "PaymentCardAddress",
        ...
        constructor(element) {
            ...
            this.countriesRegions = ko.observableArray([{ CountryCode: "NoSelection", CountryName: "Select a country/region:" }]);
            ...
            this.giftCard = ko.observableArray([{
                giftCardAmount: ko.observable<number>(0.0),
                giftCardNumber: ko.observable<string>(''),
                giftCardPin: ko.observable<string>(''),
                giftCardExpirationYear: ko.observable<number>(0),
                giftCardExpirationMonth: ko.observable<number>(0),
                giftCardBalance: ko.observable<string>(''),
                isGiftCardInfoAvailable: ko.observable<boolean>(false),
                formattedGiftCardAmount: ko.observable<string>(''),
                showBalance: ko.observable<boolean>(false),
                showPin: ko.observable<boolean>(false),
                showExpiration: ko.observable<boolean>(false),
                tenderTypeId: ko.observable<string>(''),
                cardTypeId: ko.observable<string>('')
            }]);
        ...
        private autoFillCheckout() {
            ...
            var countryRegion = address[0].getElementsByTagName("CountryRegion");
            ...
            tempAddress.ThreeLetterISORegionName = countryRegion[0].textContent;
        ...
        private geocodeCallback(geocodeResult, userData) {
            ...
            Microsoft.Maps.Events.addHandler(this.mapStoreLocator, 'viewchange', this.renderAvailableStores.bind(this));
        ...
        private selectStore(location: CommerceProxy.Entities.OrgUnitLocation) {
            ...
            if (this.hasInventoryCheck() && !this.areAllReqProductsAvailableInOrgUnit(location.OrgUnitNumber)) {
                ...
                if (this.selectedOrderDeliveryPreference() == CommerceProxy.Entities.DeliveryPreferenceType.PickupFromStore) {
                    ...
                    headerLevelDeliverySpecification.DeliveryAddress.RecordId = location.PostalAddressId;
                    this.latestHeaderLevelDeliverySpecification(headerLevelDeliverySpecification);
                }
                else if (this.selectedOrderDeliveryPreference() == CommerceProxy.Entities.DeliveryPreferenceType.DeliverItemsIndividually) {
                    ...
                    currentLineDeliverySpecification.DeliveryAddress.RecordId = location.PostalAddressId;
        ...
        private paymentCountryUpdate(srcElement) {
            // Get states when country/region is updated in payment page.
            if (!Utils.isNullOrUndefined(srcElement)) {
                ...
                switch (id) {
                    ...
                    case "OrderAddressCountry":
                        if (this.latestHeaderLevelDeliverySpecification().DeliveryAddress.ThreeLetterISORegionName != srcElement.value) {
                            ...
                            this.getStateProvinceInfoService(srcElement.value); // Get states when country/region is updated.
                            this.resetSelectedOrderShippingOptions();
                        }
        ...
        private updateHeaderLevelDeliveryPreferences(deliveryPreferenceTypeValues: number[]) {
            ...
            // Updates the available delivery preferences at order level.
        ...
        private updatePaymentTotal() {
            ...
            var totalGiftCardAmount = 0;
            this.loyaltyCardAmount = 0;

            if (this.payGiftCard()) {
                for (var index = 0; index < this.giftCard().length; index++) {
                    var currentElement = this.giftCard()[index];
                    currentElement.giftCardAmount(Utils.parseNumberFromLocaleString(currentElement.formattedGiftCardAmount()));
                    currentElement.formattedGiftCardAmount(this.formatCurrencyString(currentElement.giftCardAmount()));
                    totalGiftCardAmount += currentElement.giftCardAmount();
                }
            }
            ...
            if (this.payCreditCard()) {
                this.creditCardAmount = Utils.roundToNDigits(this.cart().TotalAmount - totalGiftCardAmount - this.loyaltyCardAmount, 3);
            ...
            this.totalAmount = Number(this.creditCardAmount + totalGiftCardAmount + this.loyaltyCardAmount);
        ...
        private validatePayments() {
            ...
            if (this.payGiftCard()) {
                ...
                var emptyCardNumber = false;
                var amountZero = false;
                var totalGiftCardAmount = 0;
                this.checkGiftCardAmountValidity = false;

                for (var index = 0; index < this.giftCard().length; index++) {
                    var currentElement = this.giftCard()[index];
                    emptyCardNumber = emptyCardNumber || Utils.isNullOrWhiteSpace(currentElement.giftCardNumber());
                    amountZero = amountZero || currentElement.giftCardAmount() == 0;
                    totalGiftCardAmount += currentElement.giftCardAmount();
                    ...
                    if (currentElement.giftCardBalance() === '') {
                        this.getGiftCardBalance(currentElement);
                    }
                }

                if (totalGiftCardAmount > this.cart().TotalAmount) {
        ...
        private createPaymentCardTenderLine() {
            ...
            if (this.payCreditCard()) {
                if (this.isCardPaymentAcceptPage()) {
                    ...
                    tenderLine.PaymentCard.House = this.paymentCardAddress().StreetNumber;
            ...
            if (this.payGiftCard()) {
                for (var index = 0; index < this.giftCard().length; index++) {
                    ...
                    var tenderLine = new CommerceProxy.Entities.CartTenderLineClass(null);
                    tenderLine.Currency = this.channelCurrencyCode;
                    tenderLine.Amount = currentElement.giftCardAmount();
                    tenderLine.TenderTypeId = currentElement.tenderTypeId();
                    tenderLine.GiftCardId = currentElement.giftCardNumber();
                    tenderLine.GiftCardPin = currentElement.giftCardPin();

                    if (currentElement.giftCardExpirationYear() > 0 && currentElement.giftCardExpirationMonth() > 0) {
                        tenderLine.GiftCardExpirationYear = currentElement.giftCardExpirationYear();
                        tenderLine.GiftCardExpirationMonth = currentElement.giftCardExpirationMonth();
                    }

                    tenderLine.CardTypeId = currentElement.cardTypeId();
         ...
         *
         * @param {Commerce.Proxy.Entities.CardTypeInfo[]} filteredCreditCardTypes The filtered list of credit card types.
         */
         ...
         * @return {Commerce.Proxy.Entities.CardTypeInfo[]} The result which returns a list of credit card types that are applicable for the card prefix.
         */
            ...
            if (typeof (message) === "string" && message.length > 0) {
                ...
                switch (messageObject.type) {
                    ...
                    case Checkout.CARDPAYMENTACCEPTPAGERESULT:
                        ...
                        this.retrieveCardPaymentAcceptResult(cardPaymentResultAccessCode, false);
                        break;

                    case Checkout.CARDPAYMENTACCEPTREDIRECT:
                        this.redirectByUsingPost(messageObject.value);
                        break;
                    default:
        ...
        private redirectByUsingPost(redirectInfoRaw: string): void {
            let redirectInfo: { url: string, data: any } = JSON.parse(redirectInfoRaw);
            let url: string = redirectInfo.url;
            let data: any = redirectInfo.data;
            let form: HTMLFormElement = document.createElement("form");
            form.method = "POST";
            form.action = url;
            console.log('preparing a redirect to: ' + url);
            ...
            console.log('submitting the redirect form...');
        ...
        /**
         * Returns the value indicating whether the page is displayed in one column.
         * @return {boolean} True when the payment accepting page should be displayed in one column. False when otherwise.
         */
        private isOneColumn(): boolean {
        ...
        private commenceCheckout() {
            ...
            let checkoutData: any = { isRedirect: !Ecommerce.Utils.isNullOrWhiteSpace(msaxValues.msax_EncodedRedirectResponse) };
            let encodedResponse: string = checkoutData.isRedirect ? JSON.parse(msaxValues.msax_EncodedRedirectResponse): null;
            

            CartWebApi.CommenceCheckout(this, checkoutData)
                .done((cart: CommerceProxy.Entities.Cart) => {
                    if (!Utils.isNullOrUndefined(cart)) {
                        CartWebApi.TriggerCartUpdateEvent(CommerceProxy.Entities.CartType.Checkout, cart);
                        this.initEntitySetCallSuccessful(InitEntitySet.CheckoutCart);

                        if (checkoutData.isRedirect) {
                            if (this.isSessionStorageSupported()) {
                                this.tenderLines = this.getTenderLinesFromSession();
                                let addressFromSession: CommerceProxy.Entities.Address = this.getAddressFromSession();
                                this.paymentCardAddress( !Utils.isNullOrUndefined(addressFromSession) ? addressFromSession : new CommerceProxy.Entities.AddressClass());
                                this.clearSessionStorage();
                             } else {
                                console.log("commenceCheckout: On redirect from payment challenge, session storage not supported by browser, couldn't retreive payment card address and tenderlines.");
                            }

                            this.loadRequiredConfigAndValidatePaymentOnRedirect(cart, encodedResponse);
                        } else {
                            this.showCheckoutFragment(this._checkoutFragments.DeliveryPreferences);
        ...
        /**
         * Loads required channel configuration and then validates payment redirect resposne.
         * @param cart The checkout cart.
         * @param encodedPaymentRedirectResponse The encoded payment redirect response.
         */
        private loadRequiredConfigAndValidatePaymentOnRedirect(cart: CommerceProxy.Entities.Cart, encodedPaymentRedirectResponse: string): void {
            let getChannelConfigurationAndTenderTypesResult: CommerceProxy.IAsyncResult<void> = this.getChannelConfigurationAndTenderTypes(cart).map((): void => void 0);
            let getDeliveryPreferencesResult: CommerceProxy.IAsyncResult<void> = this.getDeliveryPreferences().map((): void => void 0);
            let getCardTypes: CommerceProxy.IAsyncResult<void>  = this.getCardTypes().map((): void => void 0);

            CommerceProxy.VoidAsyncResult.join([getChannelConfigurationAndTenderTypesResult, getDeliveryPreferencesResult, getCardTypes]).done(() => {
                console.log("Sucessfully loaded required channel configuration, tender types, delivery preferences and card types");
                this.retrieveCardPaymentAcceptResult(encodedPaymentRedirectResponse, true);
            }).fail((errors: CommerceProxy.ProxyError[]): void => {
                console.log("Failed to load required configuration before validating redirect response.");
                let errorMessages: string[] = ErrorHelper.getErrorMessages(errors);
                this.closeDialogAndDisplayError(errorMessages, true);
            });
         }
        ...
        private getChannelConfigurationAndTenderTypes(cart: CommerceProxy.Entities.Cart): CommerceProxy.IVoidAsyncResult {
            CommerceProxy.RetailLogger.channelServiceGetChannelConfigurationStarted();
            var getChannelConfigurationAsyncResult = new CommerceProxy.VoidAsyncResult();
                ...
                .done((channelConfiguration: CommerceProxy.Entities.ChannelConfiguration) => {
                    if (!Utils.isNullOrUndefined(channelConfiguration)) {
                        ...
                        this.getTenderTypes(cart)
                            .done(() => {
                                getChannelConfigurationAsyncResult.resolve();
                            });
                        ...
                        let errors: CommerceProxy.ProxyError[] = [new CommerceProxy.ProxyError(CommerceProxy.ErrorTypeEnum.APPLICATION_ERROR, Resources.String_98)];
                        getChannelConfigurationAsyncResult.reject(errors);
                    }
            ...
            return getChannelConfigurationAsyncResult;
        }

        private getDeliveryPreferences(): CommerceProxy.IVoidAsyncResult {
            CommerceProxy.RetailLogger.checkoutServiceGetDeliveryPreferencesStarted();
            var getDeliveryPreferencesAsyncResult = new CommerceProxy.VoidAsyncResult();
            ...
            CartWebApi.GetDeliveryPreferences(this)
                .done((data: CommerceProxy.Entities.CartDeliveryPreferences) => {
                    if (!Utils.isNullOrUndefined(data)) {
                        ...
                        getDeliveryPreferencesAsyncResult.resolve();
                    } else {
                        ...
                        let errors: CommerceProxy.ProxyError[] = [new CommerceProxy.ProxyError(CommerceProxy.ErrorTypeEnum.APPLICATION_ERROR, Resources.String_98)];
                        getDeliveryPreferencesAsyncResult.reject(errors);
                    }
            ...
            return getDeliveryPreferencesAsyncResult;
        }

        private getCardTypes(): CommerceProxy.IVoidAsyncResult {
            CommerceProxy.RetailLogger.channelServiceGetCardTypesStarted();
            var getCardTypesAsyncResult = new CommerceProxy.VoidAsyncResult();
            ...
            OrgUnitWebApi.GetCardTypes(this)
                .done((data: CommerceProxy.Entities.CardTypeInfo[]) => {
                    if (!Utils.isNullOrUndefined(data)) {
                        ...
                        getCardTypesAsyncResult.resolve();
                    } else {
                        ...
                        let errors: CommerceProxy.ProxyError[] = [new CommerceProxy.ProxyError(CommerceProxy.ErrorTypeEnum.APPLICATION_ERROR, Resources.String_68)];
                        getCardTypesAsyncResult.reject(errors);
                    }
            ...
            return getCardTypesAsyncResult;
        }
        ...
        private getCountryRegionInfo(languageId: string): CommerceProxy.IVoidAsyncResult {
            CommerceProxy.RetailLogger.channelServiceGetCountryRegionInfoStarted();
            var getCountryRegionInfoAsyncResult = new CommerceProxy.VoidAsyncResult();

            StoreOperationsWebApi.GetCountryRegionInfo(languageId, this)
                .done((data: CommerceProxy.Entities.CountryRegionInfo[]) => {
                    if (Utils.hasElements(data)) {
                        this.countriesRegions(this.getSortedCountriesRegions(data));
                        this.initEntitySetCallSuccessful(InitEntitySet.CountryRegion);
                        getCountryRegionInfoAsyncResult.resolve();
                    } else {
                        ...
                        let errors: CommerceProxy.ProxyError[] = [new CommerceProxy.ProxyError(CommerceProxy.ErrorTypeEnum.APPLICATION_ERROR, Resources.String_165)];
                        getCountryRegionInfoAsyncResult.reject(errors);
                    }
            ...
            return getCountryRegionInfoAsyncResult;
        }

        private getSortedCountriesRegions(countryRegionInfoArray: CommerceProxy.Entities.CountryRegionInfo[]): { CountryCode: string; CountryName: string }[] {
                ...
                StoreOperationsWebApi.GetStateProvinceInfo(countryCode, this)
                    .done((data: CommerceProxy.Entities.StateProvinceInfo[]) => {
                        ...
                        if (this._checkoutView.find(" ." + this._checkoutFragments.DeliveryPreferences).is(":visible")) { // If in delivery preferences section
                            ...
                            if (this.selectedOrderDeliveryPreference() == CommerceProxy.Entities.DeliveryPreferenceType.ShipToAddress) {
                                // If chosen order delivery preference is ship to address and a state value exists already, and the state belongs to the selected country/region use that,
                                ...
                                // If chosen item delivery preference is ship to address and a state value exists already, and the state belongs to the selected country/region use that,
                                // else initialize with empty value.
        ...
        private getTenderTypes(cart: CommerceProxy.Entities.Cart): CommerceProxy.IVoidAsyncResult {
            CommerceProxy.RetailLogger.channelServiceGetTenderTypesStarted();
            var getTenderTypesAsyncResult = new CommerceProxy.VoidAsyncResult();
            ...
            OrgUnitWebApi.GetTenderTypes(this)
                .done((data: CommerceProxy.Entities.TenderType[]) => {
                    if (Utils.hasElements(data)) {
                        ...
                        getTenderTypesAsyncResult.resolve();
                    } else {
                        ...
                        let errors: CommerceProxy.ProxyError[] = [new CommerceProxy.ProxyError(CommerceProxy.ErrorTypeEnum.APPLICATION_ERROR, Resources.String_138)];
                        getTenderTypesAsyncResult.reject(errors);
                    }
            ...
            return getTenderTypesAsyncResult;
        }
            ...
            CartWebApi.GetCardPaymentAcceptPoint(cardPaymentAcceptSettings, this)
                .done((data: CommerceProxy.Entities.CardPaymentAcceptPoint) => {
                    if (!Utils.isNullOrUndefined(data)
                        ...
                        && (!Utils.isNullOrWhiteSpace(data.AcceptPageContent)
                            || !Utils.isNullOrWhiteSpace(data.AcceptPageUrl))) {

                        var cardPaymentAcceptUrl: string = "";
                        if (!Utils.isNullOrWhiteSpace(data.AcceptPageContent)) {
                            this.cardPaymentAcceptMessageOrigin = data.MessageOrigin;
                            this.isCardPaymentAcceptPage(true);
                            var cardPaymentAcceptIframe = <HTMLIFrameElement>document.getElementById("cardPaymentAcceptFrame");

                            cardPaymentAcceptIframe.addEventListener("load", function () {
                                cardPaymentAcceptIframe.contentWindow.document.write(data.AcceptPageContent);
                            });

                        } else {
                            cardPaymentAcceptUrl = data.AcceptPageUrl;
                            if (Utils.isNullOrWhiteSpace(data.AcceptPageSubmitUrl)) {
                                ...
                                if (this.isOneColumn()) {
                                    cardPaymentAcceptUrl = cardPaymentAcceptUrl + "&columnnumber=1";
                                } else {
                                    cardPaymentAcceptUrl = cardPaymentAcceptUrl + "&columnnumber=2";
                                }
                            }
                        ...
                        if (!(cardPaymentAcceptUrl.indexOf(data.MessageOrigin) === 0)) {
                            ...
                            if (this.isOneColumn()) {
                                ...
                                cardPaymentAcceptIframe.height = "300px";
                            }
                            LoadingOverlay.CloseLoadingDialog();
                        }
         ...
         * @param {boolean} isRedirectResponse The value indicating if this is a request to validate redirect response.
         */
        private retrieveCardPaymentAcceptResult(cardPaymentResultAccessCode: string, isRedirectResponse: boolean): void {
            ...
            StoreOperationsWebApi.RetrieveCardPaymentAcceptResult(cardPaymentResultAccessCode, this)
                .done((cardPaymentAcceptResult: CommerceProxy.Entities.CardPaymentAcceptResult) => {
                    if (!Utils.isNullOrWhiteSpace(cardPaymentAcceptResult.AdditionalContext)) {

                        if (!this.saveDataToSessionBeforeRedirect(cardPaymentAcceptResult)) {
                            return;
                        }
                        
                        var cardPaymentAcceptMessage: CardPaymentAcceptMessage = {
                            type: Checkout.CARDPAYMENTACCEPTADDITIONALCONTEXT,
                            value: cardPaymentAcceptResult.AdditionalContext
                        }
                        var cardPaymentAcceptIframe: HTMLIFrameElement = this.getPaymentFrame();
                        ...
                        this.isCardPaymentAcceptPage(true);
                        this.payCreditCard(true);
                        this.updatePaymentsOnRedirect();
                        this.processCardPaymentAcceptResult(cardPaymentAcceptResult, true);
                    } else {
                        this.processCardPaymentAcceptResult(cardPaymentAcceptResult, false);
                    } 
                    ...
                    // If redirect response validation fails, show payment info view.
        ...
        /**
         * Gets payment card address from session storage.
         * @returns {CommerceProxy.Entities.Address} Returns payment card address retrieved from session storage.
         */
        private getAddressFromSession(): CommerceProxy.Entities.Address {
            let address: string = sessionStorage.getItem(this.sessionStorageKeys.PaymentCardAdddress);
            let addressFromSession: CommerceProxy.Entities.Address = !Utils.isNullOrWhiteSpace(address) ? JSON.parse(address) : null;
            console.log("RetrieveCardPaymentAcceptResult: Retrieved payment card address from session:" + address);

            return addressFromSession;
        }

        /**
         * Gets tednerlines from session storage.
         * @returns {CommerceProxy.Entities.CartTenderLine[]} Returns cartenderlines retrieved from session storage.
         */
        private getTenderLinesFromSession(): CommerceProxy.Entities.CartTenderLine[] {
            let tenderLines: string = sessionStorage.getItem(this.sessionStorageKeys.TenderLines);
            let tenderLinesFromSession: CommerceProxy.Entities.CartTenderLine[] = !Utils.isNullOrWhiteSpace(tenderLines) ? JSON.parse(tenderLines) : [];
            console.log("RetrieveCardPaymentAcceptResult: Retrieved tenderlines from session: " + tenderLines);

            return tenderLinesFromSession;
        }

        /**
         * Clears session storage items - payment card address and tenderlines.
         */
        private clearSessionStorage(): void {
            sessionStorage.removeItem(this.sessionStorageKeys.PaymentCardAdddress);
            sessionStorage.removeItem(this.sessionStorageKeys.TenderLines);
        }

        /**
         * Checks if session storage is defined in browser.
         * @returns {boolean} Returns if session storage is supported or not.
         */
        ...
        /**
         * Updates payments on redirect.
         */
        private updatePaymentsOnRedirect(): void {

            let loyaltyTenderTypeId: string = this.getTenderTypeIdForOperationId(this.supportedTenderTypes, CommerceProxy.Entities.RetailOperation.PayLoyalty);
            let giftCardTenderTypeId: string = this.getTenderTypeIdForOperationId(this.supportedTenderTypes, CommerceProxy.Entities.RetailOperation.PayGiftCertificate);

            for (var tenderLine of this.tenderLines) {

                // Gift Card.
                if (tenderLine.TenderTypeId == giftCardTenderTypeId) {
                    console.log("updatePaymentsOnRedirect: Found gift card payment, adding it back to checkout.");
                    this.payGiftCard(true);
                    let giftCard = this.giftCard().length == 1 ? this.giftCard()[0]: null;
                    if (!Utils.isNullOrUndefined(giftCard) && giftCard.giftCardNumber() === '') {
                        giftCard.giftCardNumber(!Utils.isNullOrWhiteSpace(tenderLine.GiftCardId) ? tenderLine.GiftCardId : '');
                        giftCard.giftCardAmount(!Utils.isNullOrUndefined(tenderLine.Amount) ? tenderLine.Amount: 0.0);
                        giftCard.formattedGiftCardAmount(!Utils.isNullOrUndefined(tenderLine.Amount) ? this.formatCurrencyString(tenderLine.Amount) : '');
                        giftCard.tenderTypeId(!Utils.isNullOrWhiteSpace(tenderLine.TenderTypeId) ? tenderLine.TenderTypeId : '');
                        giftCard.cardTypeId(!Utils.isNullOrWhiteSpace(tenderLine.CardTypeId) ? tenderLine.CardTypeId : '');
                        giftCard.giftCardPin(!Utils.isNullOrWhiteSpace(tenderLine.GiftCardPin) ? tenderLine.GiftCardPin : '');
                        giftCard.giftCardExpirationYear(tenderLine.GiftCardExpirationYear >  0 ? tenderLine.GiftCardExpirationYear : 0);
                        giftCard.giftCardExpirationMonth(tenderLine.GiftCardExpirationMonth > 0 ? tenderLine.GiftCardExpirationMonth : 0);
                        giftCard.showBalance(false);
                        giftCard.showExpiration(false);
                        giftCard.showPin(false);
                        giftCard.giftCardBalance('');
                        giftCard.isGiftCardInfoAvailable(false); // Set to false as we don't have gift card balance info.
                    } else {
                        this.giftCard.push({
                            giftCardAmount: ko.observable<number>(!Utils.isNullOrUndefined(tenderLine.Amount) ? tenderLine.Amount : 0.0),
                            giftCardNumber: ko.observable<string>(!Utils.isNullOrWhiteSpace(tenderLine.GiftCardId) ? tenderLine.GiftCardId: ''),
                            giftCardPin: ko.observable<string>(!Utils.isNullOrWhiteSpace(tenderLine.GiftCardPin) ? tenderLine.GiftCardPin : ''),
                            giftCardExpirationYear: ko.observable<number>(tenderLine.GiftCardExpirationYear > 0 ? tenderLine.GiftCardExpirationYear : 0),
                            giftCardExpirationMonth: ko.observable<number>(tenderLine.GiftCardExpirationMonth > 0 ? tenderLine.GiftCardExpirationMonth : 0),
                            isGiftCardInfoAvailable: ko.observable<boolean>(false), // Set to false as we don't have gift card balance info.
                            formattedGiftCardAmount: ko.observable<string>(!Utils.isNullOrUndefined(tenderLine.Amount) ? this.formatCurrencyString(tenderLine.Amount) : ''),
                            tenderTypeId: ko.observable<string>(!Utils.isNullOrWhiteSpace(tenderLine.TenderTypeId) ? tenderLine.TenderTypeId : ''),
                            cardTypeId: ko.observable<string>(!Utils.isNullOrWhiteSpace(tenderLine.CardTypeId) ? tenderLine.CardTypeId : ''),
                            showBalance: ko.observable<boolean>(false),
                            showPin: ko.observable<boolean>(false),
                            showExpiration: ko.observable<boolean>(false),
                            giftCardBalance: ko.observable<string>('')
                ...
                // Loyalty Card.
                if (tenderLine.TenderTypeId == loyaltyTenderTypeId) {
                    console.log("updatePaymentsOnRedirect: Found loyalty card payment, adding it back to checkout.");
                    this.payLoyaltyCard(true);
                    this.loyaltyCardNumber(tenderLine.LoyaltyCardId);
                    this.loyaltyCardAmount = tenderLine.Amount;
                    this.formattedLoyaltyCardAmount(this.formatCurrencyString(this.loyaltyCardAmount));
                }
            }
        ...
        /**
         * Processes a response from RetrieveCardPaymentAcceptResult when it doesn't indicate a need in further redirect.
         * @param cardPaymentAcceptResult An instance of CartPaymentAcceptResult.
         * @param isRedirectResponse Indicates if this is a redirect response.
         */
        private processCardPaymentAcceptResult(cardPaymentAcceptResult: CommerceProxy.Entities.CardPaymentAcceptResult, isRedirectResponse: boolean): void {
            if (!Utils.isNullOrUndefined(cardPaymentAcceptResult) && !Utils.isNullOrUndefined(cardPaymentAcceptResult.TokenizedPaymentCard)) {
                ...
                if (this.setCardPaymentAcceptCardType(types)) {
                    ...
                    if (isRedirectResponse && !Utils.isNullOrUndefined(this.paymentCardAddress())) {
                        // Update address on tenderline
                        tenderLine.TokenizedPaymentCard.House = this.paymentCardAddress().StreetNumber;
                        tenderLine.TokenizedPaymentCard.Address1 = this.paymentCardAddress().Street;
                        tenderLine.TokenizedPaymentCard.City = this.paymentCardAddress().City;
                        tenderLine.TokenizedPaymentCard.State = this.paymentCardAddress().State;
                        tenderLine.TokenizedPaymentCard.Zip = this.paymentCardAddress().ZipCode;
                        tenderLine.TokenizedPaymentCard.Country = this.paymentCardAddress().ThreeLetterISORegionName;
                    } else {
                        // Update billing address from tenderline on payment card address.
                        let tokenizedPaymentCardAddress: CommerceProxy.Entities.Address = this.gePaymentCardAddressFromCardPaymentAcceptResult(cardPaymentAcceptResult);
        ...
        /**
         * Saves payment card address and tenderlines to session storage before redirecting.
         * @param cardPaymentAcceptResult The cardPaymentAcceptResult object.
         * @returns {boolean} The value indicating if the save operation succeeded or not.
         */
        private saveDataToSessionBeforeRedirect(cardPaymentAcceptResult: CommerceProxy.Entities.CardPaymentAcceptResult): boolean {
            if (this.isSessionStorageSupported()) {
                if (!Utils.isNullOrUndefined(this.tenderLines) && this.tenderLines.length > 0) {
                    let loyaltyTenderTypeId: string = this.getTenderTypeIdForOperationId(this.supportedTenderTypes, CommerceProxy.Entities.RetailOperation.PayLoyalty);
                    let giftCardTenderTypeId: string = this.getTenderTypeIdForOperationId(this.supportedTenderTypes, CommerceProxy.Entities.RetailOperation.PayGiftCertificate);
                    let tenderLines: CommerceProxy.Entities.CartTenderLine[] = this.tenderLines.filter(tenderLine => tenderLine.TenderTypeId == loyaltyTenderTypeId
                        || tenderLine.TenderTypeId == giftCardTenderTypeId);

                    let tenderLinesToStoreInSession: string= JSON.stringify(tenderLines);
                    sessionStorage.setItem(this.sessionStorageKeys.TenderLines, tenderLinesToStoreInSession);
                    console.log("Stored tenderLines in session: " + tenderLinesToStoreInSession);
                }

                if (!Utils.isNullOrUndefined(cardPaymentAcceptResult) && !Utils.isNullOrUndefined(cardPaymentAcceptResult.TokenizedPaymentCard)) {
                    let paymentCardAddress: CommerceProxy.Entities.Address = this.gePaymentCardAddressFromCardPaymentAcceptResult(cardPaymentAcceptResult);
                    paymentCardAddress.Email = this.paymentCardAddress().Email;

                    let addressToStoreInSession: string = JSON.stringify(paymentCardAddress);
                    sessionStorage.setItem(this.sessionStorageKeys.PaymentCardAdddress, addressToStoreInSession);
                    console.log("Stored payment card address in session: " + addressToStoreInSession);
                } else {
                    console.log("TokenizedPaymentCard is null in the cardPaymentAcceptResult. Unable to retrieve payment address inorder to save address to session storage before redirect.");
                    this.closeDialogAndDisplayError([Resources.String_401], true); // Sorry, something went wrong. We couldn't save required payment info before redirect.
                    return false;
                }
            } else {
                console.log("Session storage not supported by browser, couldn't save payment card address, email and tenderlines.");
                this.closeDialogAndDisplayError([Resources.String_401], true); // Sorry, something went wrong. We couldn't save required payment info before redirect.
                return false;
            }
        ...
        /**
         * Gets payment card address from cardPaymentAcceptResult.
         * @param cardPaymentAcceptResult The cardPaymentAcceptResult object.
         */
        private gePaymentCardAddressFromCardPaymentAcceptResult(cardPaymentAcceptResult: CommerceProxy.Entities.CardPaymentAcceptResult): CommerceProxy.Entities.Address {
            let paymentCardAddress: CommerceProxy.Entities.Address = new CommerceProxy.Entities.AddressClass();
            paymentCardAddress.StreetNumber = cardPaymentAcceptResult.TokenizedPaymentCard.House;
            paymentCardAddress.Street = cardPaymentAcceptResult.TokenizedPaymentCard.Address1;
            paymentCardAddress.City = cardPaymentAcceptResult.TokenizedPaymentCard.City;
            paymentCardAddress.State = cardPaymentAcceptResult.TokenizedPaymentCard.State;
            paymentCardAddress.ZipCode = cardPaymentAcceptResult.TokenizedPaymentCard.Zip;
            paymentCardAddress.ThreeLetterISORegionName = cardPaymentAcceptResult.TokenizedPaymentCard.Country;

            return paymentCardAddress;
        }

        private getPaymentFrame(): HTMLIFrameElement {
            var cardPaymentAcceptIframe: HTMLIFrameElement = <HTMLIFrameElement>document.getElementById("cardPaymentAcceptFrame");
            return cardPaymentAcceptIframe;
        }
            ...
            for (var i = 0; i < tenderTypes.length; i++) {
                switch (tenderTypes[i].OperationId) {
                    ...
                    case CommerceProxy.Entities.RetailOperation.PayGiftCertificate:
                        this.isGiftCardPaymentAllowed = true;
                        break;
                }
            }
        }
        ...
        private getGiftCardBalance(eventData: any) {
            CommerceProxy.RetailLogger.checkoutServiceGetGiftCardBalanceStarted();
            LoadingOverlay.ShowLoadingDialog();
            if (!Utils.isNullOrWhiteSpace(eventData.giftCardNumber())) {
                StoreOperationsWebApi.GetGiftCardBalance(eventData.tenderTypeId(), eventData.giftCardNumber(), eventData.giftCardPin(), eventData.giftCardExpirationYear(), eventData.giftCardExpirationMonth(), this)
                    .done((data: CommerceProxy.Entities.GiftCard) => {
                        if (!Utils.isNullOrUndefined(data)) {
                            ...
                            if (Utils.isNullOrEmpty(giftCardInResponse.Id)) {
                                eventData.isGiftCardInfoAvailable(false);
                            }
                            else {
                                ...
                                if (this.checkGiftCardAmountValidity) {
                                    ...
                                    if (Number(giftCardInResponse.Balance) < Number(eventData.giftCardAmount)) {
                                        this.closeDialogAndDisplayError([Resources.String_148], false); // Gift card does not have sufficient balance
                                    }
                                }

                                eventData.isGiftCardInfoAvailable(true);
                                eventData.giftCardBalance(this.formatCurrencyString(giftCardInResponse.Balance));
                            }

                            eventData.showBalance(true);
                            this.hideError();
                        } else {
        ...
        private setupGiftCardType(eventData: any) {
            LoadingOverlay.ShowLoadingDialog();

            try {
                var bestMatchingCardType: CommerceProxy.Entities.CardTypeInfo = null;
                var maxDigitsMatched: number = 0;

                for (var index = 0; index < this.cardTypes.length; index++) {
                    var cardType = this.cardTypes[index];

                    if (cardType.CardTypeValue !== CommerceProxy.Entities.CardType.GiftCard) {
                        continue;
                    }

                    if (this.isAssociatedCardType(cardType, eventData.giftCardNumber())) {
                        if (cardType.NumberFrom.length > maxDigitsMatched) {
                            maxDigitsMatched = cardType.NumberFrom.length;
                            bestMatchingCardType = cardType;
                        }
                    }
                }

                if (bestMatchingCardType !== null) {
                    eventData.showPin(bestMatchingCardType.IsPinRequired);
                    eventData.showExpiration(bestMatchingCardType.IsExpirationDateRequired);
                    eventData.tenderTypeId(bestMatchingCardType.PaymentMethodId);
                    eventData.cardTypeId(cardType.TypeId);
                }
                else {
                    eventData.tenderTypeId(this.getTenderTypeIdForOperationId(this.supportedTenderTypes, CommerceProxy.Entities.RetailOperation.PayGiftCertificate));
                }
            }
            finally {
                LoadingOverlay.CloseLoadingDialog();
        ...
        private applyFullGiftCardAmount(eventData: any) {
            LoadingOverlay.ShowLoadingDialog();
            if (!Utils.isNullOrWhiteSpace(eventData.giftCardNumber())) {
                CommerceProxy.RetailLogger.checkoutServiceGetGiftCardBalanceStarted();
                StoreOperationsWebApi.GetGiftCardBalance(eventData.tenderTypeId(), eventData.giftCardNumber(), eventData.giftCardPin(), eventData.giftCardExpirationYear(), eventData.giftCardExpirationMonth(), this)
                    .done((data: CommerceProxy.Entities.GiftCard) => {
                        if (!Utils.isNullOrUndefined(data)) {
                            var giftCardInResponse: CommerceProxy.Entities.GiftCard = data;
                            var totalAmount = this.cart().TotalAmount - this.totalAmount;
                            ...
                            if (Utils.isNullOrEmpty(giftCardInResponse.Id)) {
                                ...
                                eventData.isGiftCardInfoAvailable(true);
                                eventData.giftCardBalance(this.formatCurrencyString(giftCardBalance));

                                if (Number(giftCardBalance) <= Number(totalAmount)) {
                                    // Using up gift card balance.
                                    eventData.formattedGiftCardAmount(giftCardBalanceWithCurrency);
                                    eventData.giftCardAmount(giftCardBalance);
                                }
                                else {
                                    // Paying full order total using gift card.
                                    eventData.formattedGiftCardAmount(giftCardInResponse.BalanceCurrencyCode + Number(totalAmount));
                                    eventData.giftCardAmount(totalAmount);
                                    this._creditCardPanel.hide();
                                    this._paymentView.find('.msax-PayCreditCard .msax-PayCreditCardLink').show();
                                    this.payCreditCard(false);
        ...
        private addGiftCardPayment(eventData: any) {
            this.checkGiftCardAmountValidity = false;
            this.updatePaymentTotal();
        ...
        private removeGiftCardPayment(eventData: any) {
            if (this.giftCard().length === 1) {
                this.payGiftCard(false);
                eventData.giftCardAmount(0.0);
                eventData.giftCardNumber('');
                eventData.giftCardPin('');
                eventData.giftCardExpirationYear(0);
                eventData.giftCardExpirationMonth(0);
                eventData.giftCardBalance('');
                eventData.isGiftCardInfoAvailable(false);
                eventData.formattedGiftCardAmount('');
                eventData.showBalance(false);
                eventData.showPin(false);
                eventData.showExpiration(false);
            }
            else {
                this.giftCard.remove(eventData);
            }
