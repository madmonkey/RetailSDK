...
try
{
    ...
    [string]$RetailPaymentConnectorMetadata = $settings.RetailPaymentConnectorPackageMetadata

    if ([System.String]::IsNullOrWhiteSpace($RetailPaymentConnectorMetadata))
    {
        Write-Log "RetailSelfServiceMetadata is empty, exiting." $log
        exit 0
    }
    
    Write-Log ("RetailPaymentConnectorMetadata {0}" -f $RetailPaymentConnectorMetadata) $log

    $RetailPaymentConnectorPackage = ConvertFrom-Json $RetailPaymentConnectorMetadata

    #######################################################
    # Script logic
    #######################################################    
    if ($RetailPaymentConnectorPackage)
    {        
        $stopAosScript = Join-Path $RetailPaymentConnectorPackage.DestinationDirectory 'RetailHQConfiguration\Scripts\AutoStopAOS.ps1'
        Invoke-Expression $stopAosScript    
    
        Write-Log ("Processing package {0}" -f $RetailPaymentConnectorPackage.Name) $log
        $scriptLocation = Join-Path $RetailPaymentConnectorPackage.DestinationDirectory 'RetailHQConfiguration\Scripts\CopyPaymentConnectorFiles.ps1'
        
        $argumentList = @()
        $argumentList += ("-logFilePath", "`"$log`"")
        
        ${global:LASTEXITCODE} = 0
        Invoke-Expression "$scriptLocation $argumentList *>&1 | Tee-Object -Append -FilePath $log"

        $exitCode = ${global:LASTEXITCODE}
        if ($exitCode -ne 0)
        {
            Write-Log ("ExitCode: {0}" -f $exitCode) $log
            exit $exitCode
        }
    }

    Write-Log "Finished processing retail payment connector packages, exiting" $log
    exit 0
}
catch
{
...
finally
{
    if ($RetailPaymentConnectorPackage)
    {
        $startAosScript = Join-Path $RetailPaymentConnectorPackage.DestinationDirectory 'RetailHQConfiguration\Scripts\AutoStartAOS.ps1'
       & $startAosScript
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAk0qBRkYWebuwq
# VMwhU71N3ZRxCxqU6uZr1SvfavhRJKCCDYUwggYDMIID66ADAgECAhMzAAABiK9S
...
# VpRSt76aFgynN58RfP76CGxdwZskQ9YhTn9NUpofUzCBugYKKwYBBAGCNwIBDDGB
...
# Brd0JtHdHuRXMxqm1zSjQDI7d9kE2Nm/4xGcy7KfwVA4r8QbSGynOJKc2/vdExtX
# J8cRswT5byydM2Mi0bJxGVJNUgmuSlv5U1uHyFRihKT0TUqNOGwDh2b0K9ByK53y
# fl+ELPV6h5rPXdyv8Mp/Bbz0WWGoxPcr1eEGUWjwgu6ff6+8O4Q/ovoZu1N+Sdx3
# Mz8oOwH35mhJ/x6JqQMmLfgePwkvqdE9JMnIrLjjzCFPp9JMHafMeo+G4kRm5ouw
# xaARzfcPEMgc5rfd3Bi0nNUxv+FUzGH8I/+aGaPFlWSDyFiXLSSSwPKIWh5MZHqP
# 9ZYCJtfW0i8xpmV+5ftfoYIS8TCCEu0GCisGAQQBgjcDAwExghLdMIIS2QYJKoZI
...
# IOuCRzp0sv5T+V7So7uR5SrMQ/s0Icos2wAh43NSPOYJAgZfO+WMalAYEzIwMjAw
# ODIzMDQwMjQ2LjQxM1owBIACAfSggdSkgdEwgc4xCzAJBgNVBAYTAlVTMRMwEQYD
# VQQIEwpXYXNoaW5ndG9uMRAwDgYDVQQHEwdSZWRtb25kMR4wHAYDVQQKExVNaWNy
...
# DQEJAzENBgsqhkiG9w0BCRABBDAvBgkqhkiG9w0BCQQxIgQgvAnU7Sc5g1Dj9WdK
# IiSQcHcOo+XGZFSGk47MinqiN28wgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9
...
# BIIBAB8awYJpzyMMdIi7TDMlTdddVq1phxAr4LPDfMHIooj0Ty+GoifQHFs+4aqc
# nm5Fm0+91GLGM8EX8LMF3S2gqO9j+rAR0DO3wpW2+ko7umCoViHoLBJjjzbrjmGh
# fb/N+5r9tRnKvHaRFyKxuYav6VE71P2EoY6gknqU6u7YiKK0rQOKEgzwFSrzr7ZI
# eUXz7RC93GiXxMJpK73Q3bvbAjL5xQiGobqWwoQR5VBL/ABR+Hz6IH3JPU8FGn3V
# aaKny812a4I7Qwnwj2i3yJAC2lPEL4NyIjvXFF1dP4tPmLyxu6GNVUrZ8tcAWL24
# WAz2kL192s6MlZOkFOLyRij22Fo=
