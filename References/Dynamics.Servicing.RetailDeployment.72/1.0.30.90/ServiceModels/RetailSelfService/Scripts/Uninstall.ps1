...
{
    if ($TargetLocation -and (Test-Path $TargetLocation))
    {
        Remove-Item -Path $TargetLocation -Recurse -Force -ErrorAction SilentlyContinue
    }
}

Write-Log -objectToLog 'Executing Uninstall.ps1 for RetailSelfService' -LogFile $log

try
{
    $selfServiceLocationRegistryPath = 'HKLM:\SOFTWARE\Microsoft\Dynamics\7.0\RetailSelfService'
    $selfServicePackagesLocation = (Get-ItemProperty -Path $selfServiceLocationRegistryPath -Name 'SelfServicePackagesLocation' -ErrorAction SilentlyContinue).SelfServicePackagesLocation
    $selfServiceScriptsLocation = (Get-ItemProperty -Path $selfServiceLocationRegistryPath -Name 'SelfServiceScriptsLocation' -ErrorAction SilentlyContinue).SelfServiceScriptsLocation

    Write-Log -objectToLog ("Removing packages from location '{0}'" -f $selfServicePackagesLocation) -LogFile $log
    Remove-ItemRecurse -TargetLocation $selfServicePackagesLocation

    Write-Log -objectToLog ("Removing scripts from location '{0}'" -f $selfServiceScriptsLocation) -LogFile $log
    Remove-ItemRecurse -TargetLocation $selfServiceScriptsLocation

    Write-Log -objectToLog ("Removing retail self-service registry key '{0}'" -f $selfServiceLocationRegistryPath) -LogFile $log
    Remove-ItemRecurse -TargetLocation $selfServiceLocationRegistryPath
}
catch
{
    ...
    # Set a non-zero unique exit code for RetailSelfService uninstall failure
    $exitCode = 24742
    Write-Log ("Executed:{0}$ScriptLine{0}Exiting with error code $exitCode." -f [System.Environment]::NewLine) -LogFile $log
    exit $exitCode
}

Write-Log -objectToLog 'Finished executing Uninstall.ps1 for RetailSelfService' -LogFile $log


# SIG # Begin signature block
# MIIkEAYJKoZIhvcNAQcCoIIkATCCI/0CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDokkOts/Gh25xq
# 9WbVfgX9G7xlDyd3o7OSIwTHYcGUF6CCDYUwggYDMIID66ADAgECAhMzAAABiK9S
...
# BLIVh6r+eSQHWVvnupG3SZJpgkKniKudu3hsKL8IaTCBugYKKwYBBAGCNwIBDDGB
...
# cERkoMaC+lGcLCZxvAAi5hGGjljeXm4E7U1JxvefCWRjjqx1p+olnoRAga/DyzRl
# +Aml/6Hi94Khq3Pn9GcpjtnEjHNqUT5gVQ9lZ2YCHqbdgKrTuFT+GoxZ8slK3ywO
# BiIi8e40EVox4ahSnNpLzn3fDja1+eHqyzDDX318tU7Vzz7raccq4wzK/9tx9SJ9
# yFB7TFVRNJSNDewDfbP0evijFMPUC2oAN09d7e7LgdpBruXGN8aIcZGUeYb0ntlI
# PV0EFxs7lk5HQgDksFnsImMhtFrvCvmt6d6JOg2EzMbwjynUF1w/CKu77gtFDl6f
# lF3vupUNWwtiWow0jMxfoYIS8TCCEu0GCisGAQQBgjcDAwExghLdMIIS2QYJKoZI
...
# IG+BpEr6/e/0DI00BI/FRgr0ocRvDiqfBGgXiIzAAMs/AgZfO+TbzcgYEzIwMjAw
# ODIzMDQwMjQ5Ljc0NFowBIACAfSggdSkgdEwgc4xCzAJBgNVBAYTAlVTMRMwEQYD
# VQQIEwpXYXNoaW5ndG9uMRAwDgYDVQQHEwdSZWRtb25kMR4wHAYDVQQKExVNaWNy
...
# DQEJAzENBgsqhkiG9w0BCRABBDAvBgkqhkiG9w0BCQQxIgQgIpdBFCihSkJ3wtVm
# xkH849aI2Yw9SWu83ZbJXjOeuhAwgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9
...
# BIIBAE7MQrF8x4b8YZDYQRaWfpc8+hr/eMhyeLhvEapHMAQ4lJ6jwZNvdp7wK9HV
# Jpt9G7VNZa8t7U0ssy5KEN4UkL1LRmPQhCSLTohJVh5HRQ0dDFe+rGUI4Y5ptCsp
# ZgcedoSgvp43Z0noP+EaK15B/6HfxAjKje/ZSXlOSUtxgL6gHTbHbRJlDA1MoUjy
# I4gMoQBNwdqmjEsDNRdoi3Pt/z5Y3B+8Dh4b/X0iIEjRzn053AXJqcK9QsRq8VaE
# KNW6uNy/+GQcLCjIXj1zBniM1iaTTiFi80TZCJBg3GNkT9ygRTpjwG+BI1GtegGa
# BDpFb5+RDHGksGTXj79U3plCoUw=
