...
try
{
    ...
    Write-Log "Got parameters:
        RetailStorefrontPackageMetadata: $($settings.RetailStorefrontPackageMetadata)
        RetailStorefrontWebSite: $($settings.WebSiteName)"

    [string]$RetailStorefrontPackageMetadata = $settings.RetailStorefrontPackageMetadata
    [string]$RetailStorefrontWebSite = 'RetailStorefront'

    if ([System.String]::IsNullOrWhiteSpace($RetailStorefrontPackageMetadata))
    {
        Write-Log "RetailStorefrontPackageMetadata is empty, exiting."    
        exit 0
    }
    
    Write-Log ("RetailStorefrontPackageMetadata {0}" -f $RetailStorefrontPackageMetadata)

    #######################################################
    # Script logic
    #######################################################    

    $RetailStorefrontPackage = ConvertFrom-Json $RetailStorefrontPackageMetadata
    if ($RetailStorefrontPackage)
    {
        Write-Log ("Processing package {0}" -f $RetailStorefrontPackage.Name)
        $scriptLocation = Join-Path $RetailStorefrontPackage.DestinationDirectory 'RetailStorefront\Scripts\Upgrade\Core\UpdateStorefront.ps1'
        $argumentList = @()
        $argumentList += ("-RetailStorefrontWebSite", "`"$RetailStorefrontWebSite`"")
    ...
    Write-Log "Finished processing Retail Storefront package, exiting"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD+ge87Mj3Q+itv
# 8HXWGGSmmFCelgm+c8zlwUYVFbsv5aCCDYUwggYDMIID66ADAgECAhMzAAABiK9S
...
# YAYj++eRHlMDgFUR03eNhdqhNz9CQSek+Hr5KRSZ1TCBugYKKwYBBAGCNwIBDDGB
...
# rJNt3P8sQtR7d5s++XjUdSRQ79JEKPFvPyOOSAViXgcEzrgWDPLo2Ra3MW/hYV9R
# txa9U73y4pcZXPySBoekgo6feXFWFXgLhp7zcsI06EOeG9F6hT8bBHxcE320nCi3
# QTp22bhCmSsvO0jgo1zHk/1hTKM9cFVOA4Qcxpz2f+hLrEEnjg1nLphgRkh5wPCz
# yi+hugtdVkA3HPGWZcmPL6TtRi0WFzaGcJ/N0VtfmrQikonAhE99VfYNU/vT4CZQ
# +HdUvps0vn8z1JxT4E5mz9GakJyms83n+1qpfoZiC+bsz6tG2DX2IoOfY1zQTmL7
# UMVXTAh35AIPywPTjMlyoYIS8TCCEu0GCisGAQQBgjcDAwExghLdMIIS2QYJKoZI
...
# IMF0ARA2tDrc8qD95jLBiT2QwXQXQREbhQ+sXxEngQ20AgZfO+WManAYEzIwMjAw
# ODIzMDQwMjQ5LjE0NVowBIACAfSggdSkgdEwgc4xCzAJBgNVBAYTAlVTMRMwEQYD
# VQQIEwpXYXNoaW5ndG9uMRAwDgYDVQQHEwdSZWRtb25kMR4wHAYDVQQKExVNaWNy
...
# DQEJAzENBgsqhkiG9w0BCRABBDAvBgkqhkiG9w0BCQQxIgQgm5i+k1dxQ3WcR+X+
# kn2mbzY7Qg1/MLz+bsiGwdJ22iswgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9
...
# BIIBAANgZwEdj/GyMn6RLSXeuFX0Z+qR0sfnagLpreLY0yA2cKZiTX4bBn9mnCB8
# 5YTYBZwd8NmChw1jfNQtzks/r1t0MLNIwkH77gEjOMhGxAf10N2BaVPD/xVUZiRI
# MxO6jGopvX1YNuhcUdU9P8UlNYvSNM565SwYcj3VyxDh6YwvnAP79STGiEKSYWOo
# Xa1IDnGa/HCDH8C9mIz2SPLRHw0y7Xam4cRdmyEFi7tPW91xeMbZkwtzJjlsL5mG
# brIJpOPIWyYc9Ju7BMv9oPehX/Be9UJ8Bh5R/a36jm1e7/0CRoHI4TOrMOceDMDb
# E1QMcveDBagDjB0vwVLVJhiskoc=
