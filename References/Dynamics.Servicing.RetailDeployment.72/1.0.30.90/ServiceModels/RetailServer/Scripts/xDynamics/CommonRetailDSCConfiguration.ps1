function Get-DSCModuleRoot()
{
	# this is the defined DSC modules root.
...
Configuration RetailWebsite
{
    param (
        ...
        [System.String]$PackageDirectory = $(Throw 'PackageDirectory is required!'),
        [xml] $ServiceModelXml = $(Throw 'ServiceModelXml is required!'),
        [System.String]$MonitoringAssembly = $(Throw 'MonitoringAssembly is required!'))
    
    Import-DscResource -Module xWebAdministration
    Import-DSCResource -Module xDynamics    

    $webRoot=$ServiceModelXml.SelectSingleNode("//Configuration/Setting[@Name='WebRoot']").getAttribute("Value")
	
    $thumbprint=$ServiceModelXml.SelectSingleNode("//Configuration/Setting[@Name='WebsiteSslCertificateThumbprint']").getAttribute("Value")
    $hostname=([System.uri]$ServiceModelXml.SelectSingleNode("//Configuration/Setting[@Name='serviceUri']").getAttribute("Value")).Host
    
    $websiteName = $ServiceModelXml.SelectSingleNode("//Configuration/Setting[@Name='WebSiteName']").getAttribute("Value")
    $webAppPoolName = $websiteName
	$WebApplicationRegistryKeyName = "HKLM:SOFTWARE\Microsoft\Dynamics\7.0\$websiteName\$websiteName\"
	[void](New-Item $WebApplicationRegistryKeyName -Force)
	[void](New-ItemProperty -Path $WebApplicationRegistryKeyName -Name "ServiceInstallFolder" -PropertyType "String" -Value $webRoot -Force)

    File CopyRetailWebsiteFiles
    {
        Ensure="Present"
        DestinationPath=$webRoot
        Recurse=$true
        SourcePath="$PackageDirectory/Code"
        Type="Directory"
        MatchSource=$true
    }

    xWebAppPool CreateRetailAppPool
    {
        Ensure="Present"
        State="Started"
        Name="$webAppPoolName"        
        DependsOn=@("[File]CopyRetailWebsiteFiles")
    }

    DynamicsApppoolManager SetRetailApppoolIdentity
    {
        Ensure="Present"
        IdentityType=2
        ApppoolName="$webAppPoolName"        
        MonitoringAssembly=$MonitoringAssembly
    }
    ...
    xWebSite CreateRetailWebSite
    {
        ...
        DependsOn=@("[DynamicsApppoolManager]SetRetailApppoolIdentity")
    }
}

# PackageDirectory is the root of the current service unit
function Copy-RetailDSCResources([string]$PackageDirectory = $(Throw 'PackageDirectory is required!') )
{    
	Robocopy "$PackageDirectory\Scripts\xDynamics" "$env:ProgramFiles\WindowsPowerShell\Modules\xDynamics" * /E /MT > (Join-Path -Path $global:logdir -ChildPath 'RetailServiceConfiguration.log')	
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAC5x3yI9DR72Y5
# YHnODBUNL6jicP06ulqIfwPGtoN43qCCDYUwggYDMIID66ADAgECAhMzAAABiK9S
...
# MujtGAKMBzds7/E30l17C0cbEG95hqsUPW7rTbf5iTCBugYKKwYBBAGCNwIBDDGB
...
# oRqAGGh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbTANBgkqhkiG9w0BAQEFAASCAQB5
# BxL/DF1MquKeUeUkOKvhQ32P5dUBwwn7ItQJA6YLZW+GEl8oWWxFgKUTRpw7pR9l
# kvGfReSF8EG+ftgk8vCQ0CKF+Mu6Tk40Mc/pwyA6XNWD2X1AqOFucWS6sGDT2V8K
# hqdOBkiomOQ1f9fRT1N/A6Hd3Of7GyP5thhtvYW1Re9S7GiFFcmuttogbie9+mmY
# +lrPddz9PJP7+gmnmwxTCq4HGld9OmFeGwAUuFN2PXODdGpSQe9IX9AdSLWeoJ6l
# z6p5Yc/WzQHXqriDUbb9gnHk4mCRTxkjrFbKFYeZJtC/O5jHA2nmPQo783j1x6mx
# 8eMnAmHaZXlA7GB9+kzvoYIS8TCCEu0GCisGAQQBgjcDAwExghLdMIIS2QYJKoZI
...
# IDV7HSaZ/j9WVS4XnDmgEwlCqHLZ0p18BdOQ7M3aazXHAgZfO+TbzT0YEzIwMjAw
# ODIzMDQwMjQ2Ljc3N1owBIACAfSggdSkgdEwgc4xCzAJBgNVBAYTAlVTMRMwEQYD
# VQQIEwpXYXNoaW5ndG9uMRAwDgYDVQQHEwdSZWRtb25kMR4wHAYDVQQKExVNaWNy
...
# DQEJAzENBgsqhkiG9w0BCRABBDAvBgkqhkiG9w0BCQQxIgQg3zRENfI7k5KYwmOE
# oWoUCux+NYz0xWUIlWrsJEsXkkowgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9
...
# BIIBAC+OPtt1KI9nM4scMcssMhPxx3tZPR/C1QWiQ+mhrL1H2oghiGSpkP1XfKai
# f0TlwgDQpgtnDlwTaJKcEqycKX2dxZ3zj4/PSaYV3YG65fsKLOsat8fiUtAnbcNJ
# hVBU2FgOGtQxDyFdWiHfPFfgeCw4dNvvF/Anu6fof6nXyIENHuaMcfHeL1BadJFU
# jU+Cn6/qTdTeZ8HtoGyQjGNeZUtkcv3lUWnRSRN+TH+I58e0Vy9GxsKeVOAA8rox
# ZfKwerOKYJORIfTHsoCGa2CSTKPTjeB/Eolt5bJGg42x3GuQRKPRVFVuvb4de+1c
# LOm5/z/AQovsiHG4XMfkH1tNIa0=
