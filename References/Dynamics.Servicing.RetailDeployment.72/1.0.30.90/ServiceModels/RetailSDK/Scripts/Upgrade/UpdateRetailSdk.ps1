...
<#
.SYNOPSIS
    Allows a user to update Retail SDK deployment.

.DESCRIPTION
    This script provides a mechanism to update existing Retail SDK on a local system.
#>

$errorActionPreference = 'Stop'

function Get-RetailSdkInstallationDir([string]$retailSdkRegistryKey = $(throw 'retailSdkRegistryKey parameter is required!'))
{
    [string]$installationDir = [string]::Empty
    # Read installation directory from registry.
    
    Log-ActionItem ("Trying to get Retail SDK installation directory from following registry key - {0}" -f $retailSdkRegistryKey)

    $installationDirReg = Get-ItemProperty -Path $retailSdkRegistryKey -Name 'InstallationDir' -ErrorAction SilentlyContinue
    
    if($installationDirReg)
    {
        $installationDir = $installationDirReg.InstallationDir
        Log-ActionResult ("Found Retail SDK installation directory - {0}" -f $installationDir)
    }
...
function Is-UpdateApplicable(
    [string]$installationDir = $(throw 'installationDir parameter is required!'),

    [ValidateNotNullOrEmpty()]
    [string]$retailSdkUpdateCodeFolder =  $(throw 'retailSdkUpdateCodeFolder parameter is required!')
)
{
    Log-ActionItem 'Check if Retail SDK was deployed on the machine'    
    if(([string]::IsNullOrWhiteSpace($installationDir)))
    {
        Log-ActionResult 'No. Could not find installation location for Retail SDK'
        return $false
    }
    
    Log-ActionItem 'Check if Retail SDK iusntallation directory path retrieved from registry is valid'
    if(!(Test-Path -Path $installationDir))
    {
        Log-ActionResult ('No. Make sure that the installation path {0} exists' -f $installationDir)
        return $false
    }

    Log-ActionItem 'Check if Retail SDK update package constains necessary artifacts'
    if(!(Test-Path -Path $retailSdkUpdateCodeFolder))
	{
	    Log-ActionResult 'No. Could not find RetailSDK update package [Code] folder location'
        return $false
	}
...
function Get-HotfixIdFromUpdatePackage([string]$retailSdkRootFolder = $(throw 'installationDir parameter is required!'))
{
    [string]$hotFixId = 'Hotfix'
    $updatePackageRoot = Split-Path -Path $retailSdkRootFolder
    $hotFixInfoFile = Join-Path $updatePackageRoot 'HotfixInstallationInfo.xml'

    if(Test-Path -Path $hotFixInfoFile)
    {
        [xml]$content = Get-Content $hotFixInfoFile

        if(![string]::IsNullOrWhiteSpace(($content.HotfixInstallationInfo.Name)))
        {
            $hotFixId = $content.HotfixInstallationInfo.Name
        }
    }
...
try
{
    ...
    $retailSdkScriptsRootPath = (Get-Item $ScriptDir).Parent.FullName
    . (Join-Path -Path $retailSdkScriptsRootPath -ChildPath 'Common-Configuration.ps1')

    Log-TimedMessage 'Begin updating Retail SDK.'

    # Retail SDK installation folder from registry key.
    ...
    $installationDir = Get-RetailSdkInstallationDir -retailSdkRegistryKey $retailSdkRegistryKey

    # Get Retail SDK update package root folder.
    $retailSdkRootFolder = (Split-Path (Split-Path $ScriptDir -Parent) -Parent)
    $retailSdkUpdateCodeFolder = (Join-Path $retailSDKRootFolder 'Code')

    if(Is-UpdateApplicable -installationDir $installationDir -retailSdkUpdateCodeFolder $retailSdkUpdateCodeFolder)
    {
        $hotfixId = Get-HotfixIdFromUpdatePackage -retailSdkRootFolder $retailSdkRootFolder
        $retailSdkTargetFolder = Join-Path (Join-Path $installationDir 'Update') ("{0}.RetailSDKUpdate.{1}" -f $hotfixId, $(Get-Date -f yyyy-MM-dd_hh-mm-ss))
        Copy-Files -SourceDirPath $retailSdkUpdateCodeFolder -DestinationDirPath $retailSdkTargetFolder
    }

    Log-TimedMessage 'Finished updating Retail SDK.'
}
catch
{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBrxGodWQ0dRFH2
# vvLI5waW0qv+gdQ/eqSc547sxpFhUaCCDYUwggYDMIID66ADAgECAhMzAAABiK9S
...
# nAiWltqQW2h2v2AWKOHBJOdyJoRft0tvL3XrdfVUUTCBugYKKwYBBAGCNwIBDDGB
...
# T4VxPHNn9bhVneVtYneNYGt97DcjOV6RlOwkweWgPP7LfrbsBPkZurkk/Emt5n58
# vZXJpWU/gAUhUffTsDRb1uiRgiGbodCnwgEUmQUpiXfbqH8e48lkAA1+nSumSQfJ
# CeYRka8N46FH4LAtNoQ4vTFBF9jSLLgqy2NyXm7EikduSWRYfD96OtNW7OUGnOCE
# xHd+IYl/ta32gznhfaXn0ENtpOE9b4gYo56ST8+CkyfntC+Y7R2auJH1o1WrE9nW
# 0rM6u7z8Uo/E8hNLjpqX6VF2frV6x/w9f53SCYA5UDfGmEMrYIYhsre/rkeFMH5f
# CbohNUAMp8Tolop2VRzEoYIS4jCCEt4GCisGAQQBgjcDAwExghLOMIISygYJKoZI
...
# IEgHIe5bjGJPlSBfzmsGnDsukWzXJqmjGj6sO+xtTh5kAgZfPUPRUxYYEzIwMjAw
# ODIzMDQwMzQzLjIzOVowBIACAfSggdCkgc0wgcoxCzAJBgNVBAYTAlVTMQswCQYD
...
# CRABBDAvBgkqhkiG9w0BCQQxIgQgrz57x3Dn0ssm6j4aTqlpGdwj/l+JiOpe/lSt
# 0bYYLIAwgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9BCCOPhKvogUeGkGhaxtC
...
# wJXmPwWjbKm3SVm5jXZolsa3MA0GCSqGSIb3DQEBCwUABIIBACSmm4x78rAQBoxE
# dzL/4r/1RXSF+3JJ7ueuj7uWT0D6i6vKmV/MddUH07RTOeA3EWFqMJuGsShp/mUG
# +m4r2EscrQcTY6OAqsbpCdiBg8c2lXTuwjrsPqKjahQUlFq1jZa10gqpu7lheKRO
# R0w6wSRkkr6WrSZ0m+BXXKlKYe/Aejinn2SVteEyunqWOUl6MYCCNzetY3N9eWHf
# O0VRvSUarCG8v6LKm1HOVwaSA85O/l/UEcCC9kpTT+hvAjuaQ0BhyKQbubla+tqi
# i39f5pn77GoRhbVaNJoB3zT7l33LBDdVayiRHYJpMyRoBi5+GGyqU+CHURvRBM4g
# WayoFTw=
