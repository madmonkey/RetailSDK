...
# Validate Retail HQ configuration deployment

Param(
   [ValidateScript({Test-Path -Path $_ -PathType Leaf})]
   [string]$inputXml,
   [string]$log = (Join-Path -Path $env:SystemDrive -ChildPath 'RunRetailHQConfigurationDVT\ValidateRetailHQConfiguration.log')
)
...
[string]$logFile = (Join-Path -Path $logDir -ChildPath 'Debug_ValidateRetailHQConfiguration.log')

try
{
	Log-TimedMessage -logMessage "Running Retail HQ configuration DVT Script" -log $logFile
	...
	[string]$retailServerUrl = $dvtParams.DVTParameters.RetailServerUrl
	[string]$axDbName = $dvtParams.DVTParameters.AxDbName
	[string]$axDbServer = $dvtParams.DVTParameters.AxDbServer
	[string]$aosDatabaseUser = $dvtParams.DVTParameters.AosDatabaseUser
	[string]$aosDatabasePass = $dvtParams.DVTParameters.AosDatabasePass
	[string]$xmlOutputPath = $dvtParams.DVTParameters.OutputPath

	# Additional Parameters
	[string]$collectorName = 'RunRetailHQConfiguration.DVT'
	[string]$collectorType = 'PowerShellCollector'
	[string]$targetName = $collectorName
	[string]$serviceModelName = 'RetailHQConfiguration'
	...
	[string]$xmlChildPath = "{0}_RunRetailHQConfigurationDVTResults.xml" -f [System.DateTime]::Now.ToFileTimeUtc()
	[string]$xmlFilePath = (Join-Path -Path $xmlOutputPath -ChildPath $xmlChildPath)

	# Execute Retail HQ Configuration (Post Deployment Retail Server Endpoint Health Check) DVT
	Execute-ServiceModelPostDeploymentValidation -serviceModelName $serviceModelName `
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBiVgZYn3yJikb7
# tEAvqrsyNQeazhgfsMUYkN8GI1z6cqCCDYEwggX/MIID56ADAgECAhMzAAABh3IX
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIKk3KLyL
# 94Vn2H4BoHRwhgoMQ3h4iysX43aZOOJaHEaSMIG6BgorBgEEAYI3AgEMMYGrMIGo
...
# aHR0cDovL3d3dy5taWNyb3NvZnQuY29tMA0GCSqGSIb3DQEBAQUABIIBAJLKin2x
# qNYT04bqSU3WTvimTIApa5wMyzZE2UM5BZUJukr6GPsWCe/bauqV+EdSTaA1EESh
# u39mJZzdvZLF81f0DVpNzBE4Vi9VjTBKc7CqOFQRpsgfECb2M7ZhIRQo9Xy0ddWl
# cuw4aZSDGlPIZ7/nfudfKe6f64oueD1zzDgrA0AxM7luSU6i3355/JG+fktZCTgF
# wcl9mc5GWZe+HjoOJHObG+ahBkjL5jWhj75rth6tp2vB2ZUXL5IX6TXmO3RSxU3D
# aVfgucYKgL3DgtK55/wKoZqFWgqF6zmwzwmRNxTJ5Fmd7hh2JP9KhhpVH9MQ1tmd
# QOT+hHmdNs2phfqhghLxMIIS7QYKKwYBBAGCNwMDATGCEt0wghLZBgkqhkiG9w0B
# BwKgghLKMIISxgIBAzEPMA0GCWCGSAFlAwQCAQUAMIIBVQYLKoZIhvcNAQkQAQSg
# ggFEBIIBQDCCATwCAQEGCisGAQQBhFkKAwEwMTANBglghkgBZQMEAgEFAAQgzZvl
# K9WYMJAwxhLBWjL3wcfZjd0ALYT5rPmLDUUzhrACBl88B+dBVRgTMjAyMDA4MjMw
...
# MQ0GCyqGSIb3DQEJEAEEMC8GCSqGSIb3DQEJBDEiBCDRpQudMw0qf7tWF/blckh9
# jo4YcOR2TpWj4/pkOfiDaDCB+gYLKoZIhvcNAQkQAi8xgeowgecwgeQwgb0EIGQn
...
# i4cl0UUk+T7SVSofaKi/J4XnarV54uCRRJ9/vvXkOOJOyAPsllpTpCXtexI+cK71
# mPeJVLOuKIUEWOjI0nzbzGBGh0vaWwLlvWWXkXsomJiZNaxy/qgKySO7oiWXlqro
# RbQ18dfvg//o14PIRrjoaO5SnFAiicYA0dFZYtx0l/4QmhAcRWuuBfOWG09qbzRr
# 1OF0vdsaaroD1G8hvOu9lL/x9nBnDwl+lC4kPfKb4uArgQJesTCSH4YX7sljgbUj
# kPmHWTD5UQBhjEVR1scQtS3KZBXjk/EzrGnifSeCtI03LAR9H4rOonQthq6+7AmW
# Rke+LtLy5dgL0uREERFZSw==
