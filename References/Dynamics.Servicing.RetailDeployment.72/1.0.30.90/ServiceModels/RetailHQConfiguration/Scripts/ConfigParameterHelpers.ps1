function Get-OptionalBoolParameter
{
    param(
        [Parameter(Mandatory=$true)]
        [PSObject]$ParameterSet,
        [Parameter(Mandatory=$true)]
        [string]$ParameterName,
        [bool]$DefaultValue = $false,
        [switch]$IgnoreEmptyStringValue
    )


    $returnValue = $null

    $storedValue = $ParameterSet.$ParameterName

    if($storedValue -eq $null)
    {
        # if parameter is not present, use default value
    ...
    else
    {
        ...
        elseif ($storedValue -is [string])
        {
            # handle case of value being "emtpy"
            if([string]::IsNullOrWhiteSpace($storedValue))
            {
                if($IgnoreEmptyStringValue)
                {
                    # if caller indicated that empty is the same as not-present, then use the
                    # default value
                    $returnValue = $DefaultValue
                }
                else
                {
                    throw "Empty value for parameter that did not allow it.  ParameterName = $parameterName"
                }
            }
            else
            {
                $returnValue = [bool]::Parse($storedValue)
            }
        }
        else
        {
            throw "Unsupported data type $($storedValue.GetType().Name) specified."
        }
    }
...
function Invoke-IfParameterExists
{
    param(
        [Parameter(Mandatory=$true)]
    ...
    $parameterValue = $ParameterSet.$ParameterName

    if($parameterValue -ne $null -and
        ((-not [String]::IsNullOrWhiteSpace($parameterValue)) -or (-not $IgnoreEmptyStringValue)) )
    {
        $_ = $parameterValue
        $parameterValue | & $ScriptBlock
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA3MBR3vCxPia8N
# e9b/seBH7Ure2tsmxS6shLdAepJEFqCCDYUwggYDMIID66ADAgECAhMzAAABiK9S
...
# TFCaaFb8px5JhlfQDI2n7UjEM6SQGIYFRjeFQPgpAzCBugYKKwYBBAGCNwIBDDGB
...
# oRqAGGh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbTANBgkqhkiG9w0BAQEFAASCAQAW
# 5nVOno2EkdB0oCE3wxDy6Ww1d7wuA61+MrdmlwcvK6elEBGe5aMLT8AprzTOTnR/
# K6vJo+2/EUY9+EmBuXtNRggUVlrNX0WmSZ5g/c+LFG9rDVOFmN5AcNU8+k5HYwHl
# iSY3d98U97IK6qbcwmoXHsFc8Qy5e0y9Thnt4VTZGjXz3X9FbTAQX30hE2QoNfeJ
# 2RFHZDy6utQT+MMLckxpqWzqCu0skOZSloeUp10fpR21ubjP//njECdyvyDQgVZt
# LbVkQjMez1jlryjIVxv+Q8V0KIIo9WyU+uyWshpgRxBAFSltfXtTq7GsjB4cPhZ+
# CXVBmCuRPj4fp7Kj1v1doYIS5TCCEuEGCisGAQQBgjcDAwExghLRMIISzQYJKoZI
...
# INYXfLo9260h++99AF+30S+QtqxkrrhdQRx4KtFBziQcAgZfPVxFLAQYEzIwMjAw
# ODIzMDQwMzQ0LjQ5NFowBIACAfSggdCkgc0wgcoxCzAJBgNVBAYTAlVTMQswCQYD
...
# 9w0BCRABBDAvBgkqhkiG9w0BCQQxIgQgNx/MfiDxiKeM+b6wJkPeOAq9W6tMw9to
# CoxGNssaR0UwgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9BCAkErCCv+giGoqp
...
# 6ysYf/NGbUI4EY8DaTxh143FJO9UMA0GCSqGSIb3DQEBCwUABIIBAGzPNAPV8Igg
# QxrQLKSAq8ojI48L1vx9YmLu0kUHfYI4oSltQRzzN6ArEOm5I7wP8+1+MgCzlxP0
# I9KydjGzYu5hxWhSNuVyikIKz2/HL/VPyQ/oszl/ZlgGHwtzPXvW2Juhjw0470BL
# wWBhkHHHD75nRhyZ8gOnRpgo5pcxxgIx4mmgoRjNzOJP0Yy0NjjlLFbbcPAu3Il7
# Nv1JE4l0TpFaXA8xOjAoiT73I0LjJwoZzVY8UlRx0IO65VERBxj73VwwgSJjn25X
# HZ+E/Drdya1hBLTvEchOeglGWqoeiLoFl1waQtiONy/5iR4an8D7DUmdzQjTTRSn
# c+526zsPNzg=
