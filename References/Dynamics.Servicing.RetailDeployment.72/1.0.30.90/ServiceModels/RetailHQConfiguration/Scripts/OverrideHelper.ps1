# Helper set of functions for execuring file based overrides (used post-deployment configuration/specialization steps)
function Set-Overrides()
{
    ...
    (
        ...
        [switch]$BackupInTimestampedDirectory
    ...
    if($OverridesDefinition.Overrides -eq $null -or
        $OverridesDefinition.Overrides.Length -eq $null -or
        $OverridesDefinition.Overrides.Length -eq 0)
    {
        # There's nothing to process, bail out
        return
    }

    # if backup on ,then if generate backup sub-directory
    $BackupDirectory = $BackupDirectory | Expand-EnvVars

    if($BackupFiles)
    {
        if($BackupDirectory -eq $null)
        {
          throw "BackupDirectory not specified, but BackupFiles option was set to true"
        }


        if($BackupInTimestampedDirectory)
        {
            $timeStampMarker =  [DateTime]::Now.ToString("s").Replace(":", ".")
            $BackupDirectory = Join-Path -Path $BackupDirectory -ChildPath $timeStampMarker
        }

        if(-not (Test-Path $BackupDirectory))
        {
            # Directory doesn't exist, check that it's a valid string 'path' value before attempting to create it
            if(-not (Test-Path $BackupDirectory -IsValid))
            {
                throw "Invalid path specified for BackupDirectory '$BackupDirectory'"
            }
    ...
    foreach($overrideEntry in $OverridesDefinition.Overrides)
    {
        if($overrideEntry -eq $null){ continue }

        $overrideAction = $overrideEntry.overrideAction

        if([String]::IsNullOrWhiteSpace($overrideAction))
        {
            Write-Warning "Skipping override entry: no overrideAction specified"
            continue
        }
        elseif($overrideAction -eq "CopyFile")
        {
            $sourceFile = $overrideEntry.sourceFile | Expand-EnvVars
            $targetFile = $overrideEntry.targetFile | Expand-EnvVars

            if([String]::IsNullOrWhiteSpace($sourceFile))
            {
                Write-Warning "Skipping override entry: SourceFile not specified"
                continue
            }

            if(-not (Test-Path $sourceFile))
            {
                Write-Warning "Skipping override entry: SourceFile not found '$($sourceFile)'"
                continue
            }
            ...
            {
                Write-Warning "Skipping override entry: TargetFile not specified (Source = $($sourceFile))"
                continue
            }

            if(-not (Test-Path $targetFile))
            {
                # only fail this override/overwrite entry if the target file is required to be there, otherwise log and keep going with the copy
                if($overrideEntry.requireTargetFilePresent -eq $true) 
                {
                    Write-Warning "Skipping override entry: TargetFile not found '$($targetFile)'"
                    continue
                }
                else 
                {
                    Write-Warning "Backup skipped: TargetFile not found '$($targetFile)'"
                }
            }
            elseif($BackupFiles)
            {
                # will make a copy of the file that will be replaced in the
                # backup directory, and overwrite any previous backups (use timestamp if concerned)
                Copy-Item -Force $targetFile -Destination $backupDirectory
            }

            # execute the overwrite
            Copy-Item -Force $sourceFile -Destination $targetFile
        }
        else
        {
            Write-Warning "Skipping override entry: Unknown OverrideAction '$($overrideAction)'"
            continue # it's the last line in the loop for now, continue is just a safeguard in case more lines are added below later
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC