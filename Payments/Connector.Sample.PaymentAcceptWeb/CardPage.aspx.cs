    ...
    namespace Retail.SampleConnector.PaymentAcceptWeb
    {
        ...
        public partial class CardPage : System.Web.UI.Page
        {
            ...
            private void InitilizePageControls()
            {
                ...
                for (int i = 1; i <= 12; i++)
                {
                    this.ExpirationMonthDropDownList.Items.Add(new ListItem(string.Format("{0:D2} - {1}", i, monthNames[i - 1]), string.Format("{0}", i)));
                }
                ...
                if (this.BillingAddressPanel.Visible)
                {
                    ...
                    // TO DO: Filter the countries/regions down to the list you support.
                    ...
                    var dataManager = new DataManager();
                    IEnumerable<CountryOrRegion> countriesRegions = dataManager.GetCountryRegionListByLocale(this.entry.EntryLocale);
                    countriesRegions = countriesRegions.OrderBy(c => c.LongName).ToList();
                    foreach (var countryRegion in countriesRegions)
                    {
                        this.CountryRegionDropDownList.Items.Add(new ListItem(countryRegion.LongName, countryRegion.TwoLetterCode));
                    }
