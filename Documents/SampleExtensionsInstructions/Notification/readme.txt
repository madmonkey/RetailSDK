Sample overview:
The sample shows how to expand an existing CRT service with additional functionality. In this case, the notification service can be extended to return notifications for any retail operation.
...
1. Enable and test CRT sample code:
    ...
    - Register the CRT change in commerceruntime.ext.config (Note: Please DO NOT edit commerceruntime.config file. This file is not meant for any customizations.).
    - add the new CRT extension dll to customization.settings (@(ISV_CommerceRuntime_CustomizableFile))
    - uncomment the related code in CRT test host for the Notification sample
